<div id="profile" class="card-section resume-section animated-section" data-animate="slide-left">
  <h2>Jeremy Voegele</h2>
  <p class="text-muted">IT & Mechanical Systems Technician | U.S. Navy Veteran | Linux & Cloud Enthusiast</p>
  <p>
    Tenacious and resourceful technician with 10 years of honorable U.S. Navy service, currently in the Navy Reserves.
    Proven expertise in Linux systems, Windows Server, Active Directory, and enterprise automation tools.
    Passionate about cloud infrastructure, homelab environments, and building full-stack tools to support technical teams.
    Security+ certified and experienced in hands-on leadership across diverse operational environments.
  </p>

  <div class="mt-4">
    <h5 class="mb-3">📬 Contact</h5>
    <ul class="list-unstyled">
      <li class="mb-2">
        <strong>Email:</strong>
        <a href="mailto:jeremy.voegele@gmail.com" class="text-decoration-none text-light">jeremy.voegele@gmail.com</a>
      </li>
      <li class="mb-2">
        <strong>Phone:</strong> <span class="text-light">(682) 300-1197</span>
      </li>
      <li class="mb-2">
        <strong>GitHub:</strong>
        <a href="https://github.com/Ceaserxl" target="_blank" class="text-decoration-none text-light">github.com/Ceaserxl</a>
      </li>
      <li class="mb-2">
        <strong>LinkedIn:</strong>
        <a href="https://www.linkedin.com/in/jeremy-voegele-5062951b7/" target="_blank" class="text-decoration-none text-light">linkedin.com/in/jeremy-voegele</a>
      </li>
      <li class="mb-2">
        <strong>Location:</strong> <span class="text-light">San Diego, CA</span>
      </li>
    </ul>
  </div>
</div>


<div id="work" class="card-section resume-section animated-section" data-animate="fade-scale">
  <h4>Work Experience</h4>
  <div class="timeline-line">
    <div class="timeline-item">
      <strong>Dec 2023 – Present:</strong> U.S. Navy Reserves – Information Systems Technician
      <ul>
        <li>Managed 200+ user accounts via Active Directory with group policy enforcement</li>
        <li>Maintained 300+ workstations and tracked 1,000+ inventory/configuration records</li>
        <li>Performed Windows maintenance: imaging, patching, troubleshooting</li>
        <li>Updated 500+ service records and system logs for readiness/audits</li>
      </ul>
    </div>
    <div class="timeline-item">
      <strong>Aug 2014 – Dec 2023:</strong> U.S. Navy – Aviation Support Equipment Technician
      <ul>
        <li>Led 30+ junior personnel, taught licensing classes, ensured QA/safety compliance</li>
        <li>Completed 1,200+ maintenance tasks totaling 1,600+ hours across diesel, hydraulic, and electrical systems</li>
        <li>Operated forklifts/cranes (1,000+ hrs) for 4,000–20,000 lbs equipment</li>
        <li>Supported two overseas deployments and managed 3,000+ inspection records</li>
        <li>Processed $1.2M+ in DoD travel funds using custom MS Access automation</li>
      </ul>
    </div>
  </div>
</div>

<div id="experience" class="card-section resume-section animated-section" data-animate="rotate-list">
  <h4>Technical Experience</h4>
  <ul>
    <li><strong>Homelab Systems Admin:</strong>
      <ul>
        <li>Built GPU-enabled Proxmox cluster for virtual services</li>
        <li>Hosted Stable Diffusion, ChatGPT APIs with GPU acceleration</li>
        <li>Automated backups/cloud sync using Python + RClone</li>
        <li>Tuned 2.5GbE/10GbE networks for max VM throughput</li>
        <li>Developed custom Discord bot with AI microservices</li>
      </ul>
    </li>
  </ul>
</div>

<div id="volunteer" class="card-section resume-section animated-section" data-animate="rotate-list">
  <h4>Volunteer IT Work</h4>
  <ul>
    <li><strong>Volunteer IT Technician:</strong>
      <ul>
        <li>Set up LANs and configured user systems for local orgs</li>
        <li>Performed PC repair, OS installs, file server deployments</li>
        <li>Wrote Python scripts and trained users in best practices</li>
      </ul>
    </li>
  </ul>
</div>

<div id="projects" class="card-section resume-section animated-section" data-animate="rotate-list">
  <h4>Personal Projects</h4>
  <ul>
    <li><strong>Discord Bot (2024–Present):</strong>
      <ul>
        <li>Built a modular Python bot with AI, image generation, media control</li>
        <li>Integrated ChatGPT and image tools with HTTP + WebSocket</li>
        <li>Designed Discord Views for dynamic upscaling and delete actions</li>
      </ul>
    </li>
    <li><strong>Intro to Machine Learning (2022):</strong>
      <ul>
        <li>Created a 2D self-driving car in PyTorch using neural networks</li>
        <li>Trained via reward loop with forward propagation and loss functions</li>
        <li>Built custom training visualizations</li>
      </ul>
    </li>
  </ul>
</div>

<div id="certs" class="card-section resume-section animated-section" data-animate="glow-pulse">
  <h4>Certifications</h4>
  <ul>
    <li>CompTIA Security+ (Obtained March 2025)</li>
    <li>Network+ (In Progress)</li>
    <li>A+ (In Progress)</li>
  </ul>
</div>

<div id="education" class="card-section resume-section animated-section" data-animate="cascade">
  <h4>Education</h4>
  <ul>
    <li>Goal: Transfer to San Diego State University – B.S. Computer Science</li>
    <li><strong>Southwestern College (Starting Aug 2025):</strong> Associate of Science – Computer Science</li>
    <li><strong>CNATTU – Jacksonville, FL:</strong> Shipboard Support Equipment, Maintenance Manager (2017 & 2020)</li>
    <li><strong>CNATTU – Jacksonville, FL:</strong> Large Tow Tractor School (2015)</li>
    <li><strong>NATTC – Pensacola, FL:</strong> Aviation Support Equipment School (2014–2015)</li>
    <li><strong>Boswell High School – Saginaw, TX:</strong> High School Diploma (2014)</li>
  </ul>
</div>

<div id="awards" class="card-section resume-section animated-section award-shimmer" data-animate="cascade">
  <h4>Awards</h4>
  <ul>
    <li>Good Conduct Medal ×3</li>
    <li>Humanitarian Service Medal ×2</li>
    <li>Navy "E" Ribbon</li>
    <li>Global War on Terrorism Expeditionary Medal</li>
    <li>Sea Service Deployment Ribbon</li>
  </ul>
</div>
